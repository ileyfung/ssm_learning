				...
				<select id="doorId" name="doorId">
					<c:forEach items="${doors}" var="door">
						<option value="${door.id}"
								<c:if test="${order.doorId == door.id}">selected</c:if>
						>${door.name}</option>
					</c:forEach>
				</select>
			</td>
		...
		<tr>
			...
			<td>
				<input type="text" name="orderTime" 
						value='<fmt:formatDate value="${order.orderTime}" pattern="yyyy/MM/dd HH:mm:ss"></fmt:formatDate>'/>
			</td>
		</tr>
		<tr>
			<td>结账时间</td>
			<td>
				<input type="text" name="orderTime" 
						value='<fmt:formatDate value="${order.payTime}" pattern="yyyy/MM/dd HH:mm:ss"></fmt:formatDate>'/>
			</td>
		</tr>
		<tr>
			<td>支付方式</td>
